...
class GraphConvolution(nn.Module):
    ...
    def __init__(self, in_features, out_features, bias=True, node_n=48):
        ...
        所以在参数优化的时候可以进行优化的)，
        '''
        self.weight = Parameter(torch.FloatTensor(in_features, out_features))  # 权重设置为parameter类
        self.att = Parameter(torch.FloatTensor(node_n, node_n))  # 可学习的加权邻接矩阵
    ...
    # 前向函数
    def forward(self, x):
        support = torch.matmul(x, self.weight)  # 高维矩阵相乘
        y = torch.matmul(self.att, support)  # 三个矩阵相乘input X与权重W相乘，然后adj矩阵与 他们的积稀疏乘。(AHW)三个举证相乘
        if self.bias is not None:
